...
# Ensure Streamlit serves on a specific port
import sys
if len(sys.argv) == 1:
    import os
...
def load_conversation_data(custom_path=None):
    ...
    This function dynamically locates and loads conversation data using the following priority:
    ...
    if isinstance(data, dict) and any("events" in val for val in data.values() if isinstance(val, dict)):
        conversations = []
        for tracker_id, tracker_data in data.items():
            ...
            if messages:
                ...
                conversations.append({
                    ...
                    "num_user_messages": sum(1 for msg in messages if msg["type"] == "user"),
                    "num_bot_messages": sum(1 for msg in messages if msg["type"] == "bot"),
                    "num_actions": sum(1 for msg in messages if msg["type"] == "action"),
                })
...
if not conversations:
    st.warning("No conversation data found. Please make sure your conversation data is available.")
    ...
    if "messages" in conv:
        ...
        if "duration" not in conv:
            ...
            if len(conv["messages"]) >= 2:
                try:
                    first_msg_time = min(msg.get("timestamp", 0) for msg in conv["messages"])
                    last_msg_time = max(msg.get("timestamp", 0) for msg in conv["messages"])
...
with col1:
    st.metric(
        ...
        "+{}%".format(round((len(filtered_conversations) / max(1, len(conversations))) * 100 - 100)) 
        if len(conversations) > 0 else "+0%"
    )

with col2:
    avg_msgs = np.mean([conv.get("num_user_messages", 0) for conv in filtered_conversations]) if filtered_conversations else 0
    st.metric("Avg. Messages per Conversation", f"{avg_msgs:.1f}")

with col3:
    avg_duration = np.mean([conv.get("duration", 0) for conv in filtered_conversations]) if filtered_conversations else 0
    st.metric("Avg. Duration (seconds)", f"{avg_duration:.1f}")

with col4:
    if filtered_conversations and any("feedback" in conv for conv in filtered_conversations):
...
if filtered_conversations:
    ...
    conversation_dates = [conv["timestamp"].date() if isinstance(conv["timestamp"], datetime) 
                         else datetime.fromtimestamp(conv["timestamp"]).date() 
                         if isinstance(conv["timestamp"], (int, float)) else datetime.now().date() 
                         for conv in filtered_conversations]
    ...
    chart = alt.Chart(date_df).mark_line().encode(
    ...
    with tab1:
        ...
        if all_user_messages:
            ...
            chart = alt.Chart(message_df).mark_bar().encode(
                x=alt.X('Count:Q'),
                y=alt.Y('Message:N', sort='-x'),
                tooltip=['Message:N', 'Count:Q']
            ).properties(
                title='Top 10 User Messages',
                height=400
            )
    ...
    with tab3:
        ...
        if filtered_conversations:
            ...
            for i, conv in enumerate(filtered_conversations):
                ...
                duration_display = f"{duration:.1f} sec" if duration < 60 else f"{duration/60:.1f} min"
            ...
            conv_display = [f"Conversation {i+1} - {conv.get('user_id', 'Unknown User')}" 
                           for i, conv in enumerate(filtered_conversations)]
            ...
            selected_id = st.selectbox("Select conversation to view:", conv_ids, 
            ...
            if selected_id and selected_id in conv_map:
                ...
                if 0 <= selected_index < len(filtered_conversations):
                    ...
                    duration_display = f"{duration:.1f} seconds" if duration < 60 else f"{duration/60:.1f} minutes"
