...
def load_conversations(path: str) -> list:
    try:
        ...
        if conversations:
            print(
                f"First conversation ID: {conversations[0].get('sender_id', 'Unknown')}"
            )
...
def compute_summary(convs: list) -> pd.DataFrame:
    rows = []
    for conv in convs:
        sid = conv["sender_id"]
        ...
        fallback_count = sum(
            ...
            if (
                e.get("event") == "action"
                and e.get("name") == "action_default_fallback"
            )
            or (
                e.get("event") == "user"
                and e.get("parse_data", {}).get("intent", {}).get("name")
                == "nlu_fallback"
            )
        )
        rows.append(
...
def compute_intents(convs: list) -> pd.DataFrame:
    intents = [
    ...
    return (
        ...
        .head(10)
        .reset_index()
        .rename(columns={"index": "intent", 0: "count"})
    )


def compute_response_times(convs: list) -> tuple[pd.DataFrame, list]:
    rows = []
    all_rts = []
    for conv in convs:
        sid = conv["sender_id"]
        ...
        for i, e in enumerate(events):
            if e.get("event") == "user":
                ...
                for ne in events[i + 1 :]:  # noqa: E203
                    if ne.get("event") == "bot":
...
def make_dashboard(
    ...
    fig.suptitle(
        f"Rasa Conversation Dashboard — "
        f'{datetime.now(ZoneInfo("Europe/Berlin")).strftime("%Y-%m-%d %H:%M:%S %Z")}',
        fontsize=20,
        y=0.98,
    )
    ...
    axes[0, 0].axis("off")
    axes[0, 0].table(
        cellText=df_summary.values, colLabels=df_summary.columns, loc="center"
    )
    ...
    axes[1, 0].hist(
        df_summary["fallbacks"], bins=int(df_summary["fallbacks"].max()) + 1
    )
