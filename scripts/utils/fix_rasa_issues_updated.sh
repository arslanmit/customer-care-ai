...
show_help() {
    echo "Usage: ./fix_rasa_issues_updated.sh [options]"
...
#!/bin/bash
# Usage: ./scripts/utils/fix_rasa_issues_updated.sh [options]
# Run with -h or --help for usage information

# =========================================================
# RASA Troubleshooting and Fix Script (Updated)
# =========================================================
# This script:
...
# Correct file paths for your project structure
...
backup_files() {
    ...
    if file_exists "$CONFIG_FILE"; then
        cp "$CONFIG_FILE" "backups/config-$timestamp.yml"
...
apply_fallback_config() {
    ...
    if [ -f "fallback_config/enhanced_fallbacks.yml" ]; then
        ...
        if [ -f "$CONFIG_FILE" ]; then
            ...
            if grep -q "policies:" "$CONFIG_FILE"; then
                ...
                cat "$FIXES_DIR/policies.yml" >> "$CONFIG_FILE"
...
fix_testing() {
    ...
    if [ ! -f "tests/test_stories.yml" ]; then
        ...
        cat > "tests/test_stories.yml" << EOF
version: "3.1"
...
EOF
        echo -e "${GREEN}✅ Created test stories file${NC}"
    else
        echo -e "${GREEN}✅ Test stories file already exists${NC}"
    fi
}
...
check_analytics() {
    section "Checking analytics dashboard"
    
    echo -e "${YELLOW}Verifying analytics dashboard setup${NC}"
    if [ -f "analytics_dashboard.py" ]; then
        echo -e "${GREEN}✅ Analytics dashboard file exists${NC}"
        ...
        if pip show streamlit &>/dev/null; then
            ...
            if pgrep -f "streamlit run analytics_dashboard.py" &>/dev/null; then
                echo -e "${GREEN}✅ Analytics dashboard is already running${NC}"
                echo -e "${BLUE}💡 Access dashboard at: http://localhost:8501${NC}"
            else
                echo -e "${YELLOW}Starting analytics dashboard${NC}"
                streamlit run analytics_dashboard.py &>/dev/null &
                echo -e "${GREEN}✅ Analytics dashboard started${NC}"
                echo -e "${BLUE}💡 Access dashboard at: http://localhost:8501${NC}"
            fi
        else
            echo -e "${YELLOW}Streamlit not installed. Installing...${NC}"
            pip install -q streamlit pandas matplotlib altair
        ...
        echo -e "${RED}❌ Analytics dashboard file not found${NC}"
    fi
}
